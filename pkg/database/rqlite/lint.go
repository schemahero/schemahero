...
func ValidateSchema(tableName string, rqliteTableSchema *schemasv1alpha4.RqliteTableSchema) error {
	if rqliteTableSchema == nil {
		return errors.New("rqlite table schema required")
	}
	...
	if len(rqliteTableSchema.Columns) == 0 {
	...
	if len(primaryKeys) > 0 && rqliteTableSchema.PrimaryKey != nil {
		return errors.New("primary key cannot be specified in both column constraints and table constraints")
	}

	if rqliteTableSchema.PrimaryKey != nil {
		for _, column := range rqliteTableSchema.PrimaryKey {
	...
	if rqliteTableSchema.ForeignKeys != nil {
		for _, fk := range rqliteTableSchema.ForeignKeys {
			if len(fk.Columns) == 0 {
	...
	// Validate indexes
	if rqliteTableSchema.Indexes != nil {
		for _, index := range rqliteTableSchema.Indexes {
...
func validateColumnType(column schemasv1alpha4.RqliteTableColumn) error {
	// List of RQLite/SQLite data types (storage classes)
